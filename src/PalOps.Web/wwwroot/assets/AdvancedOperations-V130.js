import{C as computed,G as onMounted,P as h,gt as ref,mt as reactive,j as defineComponent,r as baseApi}from"./vue-i18n-CIubIkY4.js";
...
import{t as formatDateTime}from"./format-CGzLUnlF.js";
...
function renderAdvancedPage(name,context){const renderers={DiagnosticCenterPage:renderDiagnosticCenter,IncidentCenterPage:renderIncidentCenter,PlayerInsightsPage:renderPlayerInsights,WorldGovernancePage:renderWorldGovernance,DisasterRecoveryPage:renderDisasterRecovery,UpdateCenterPage:renderUpdateCenter,ConfigurationVersionsPage:renderConfigurationVersions,OperationsPlaybooksPage:renderOperationsPlaybooks,SecurityCenterPage:renderSecurityCenter,IntegrationCenterPage:renderIntegrationCenter};const renderer=renderers[name];if(!renderer)return advancedAlert('未知高级运维页面：'+name,'danger');const page=renderer(context);const notice=advancedReadinessNotice(context);if(!notice)return page;const children=Array.isArray(page.children)?page.children:[];return h('section',page.props,[children[0],notice,...children.slice(1)])}
...
const metrics = computed(() => [
    ...
    { label: '差异状态', value: diff.value ? (diff.value.identical ? '一致' : `${diff.value.changedKeys.length} 项变化`) : '未比较', tone: diff.value?.identical ? 'success' : diff.value ? 'warning' : undefined },
]);
function message(cause) { return cause instanceof Error ? cause.message : String(cause); }
async function load() {
    readinessState.clearError();
...
export{DiagnosticCenterPage,IncidentCenterPage,PlayerInsightsPage,WorldGovernancePage,DisasterRecoveryPage,UpdateCenterPage,ConfigurationVersionsPage,OperationsPlaybooksPage,SecurityCenterPage,IntegrationCenterPage};

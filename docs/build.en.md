...
The fixed-POI baseline is committed as reviewed JSON under `frontend-vue/public/map/data`. The repository no longer contains a Python generator. After editing POI data, run `npm run verify:map-complete-local` and `npm run build`. The cacheable assets remain outside `src/features/map`, contain 1,251 records per runtime locale, and keep all 30 categories nonempty.

...
**1.3.0** 将 PalServer 生命周期、实时监控、离线世界地图、玩家与公会、物资发放、存档解析、配置维护、诊断治理、灾备、安全与外部集成统一到一个现代化 Web 工作台。
...
![PalOps Web V1.3.0 运行概览](docs/images/zh-CN/overview-dashboard.webp)

> **截图说明：** README 中的全部界面图均来自 V1.3.0 当前前端构建，使用专用伪数据、虚构玩家/公会和保留地址生成。截图不包含真实服务器 IP、世界 ID、账号、Token、Webhook、Cookie、存档路径或生产数据。

## V1.3.0 重点更新

- **统一配置依赖提示**：所有依赖 Palworld REST、RCON、PalDefender、存档路径、存档索引或备份目录的页面都会显示内嵌就绪提示；配置恢复可用后提示自动消失。
- **首次使用引导**：系统设置新增分步骤配置清单、完成度、必须项/可选项说明和直接跳转入口，避免未配置状态下连续报错。
- **诊断与故障闭环**：新增诊断中心、健康检查、支持包、告警规则、故障确认、分派、恢复和时间线。
- **玩家与世界治理**：新增玩家时间线、活跃度/风险洞察、据点治理、孤儿据点和长期不活跃候选分析。
- **平台级运维能力**：新增灾备中心、更新中心、配置版本库、运维剧本、安全中心和对外集成中心。
- **安全边界强化**：高风险确认、一次性 API Token、作用域、过期/吊销、HTTPS 订阅、SSRF 防护、重试与审计。
- **发布体验完善**：中英文 README 和所有功能模块截图全部重制，物资发放三步流程独立展示。
...
未完成必要配置时，相关页面会显示内嵌说明并阻止不安全操作；配置检测恢复后提示会自动消失。
